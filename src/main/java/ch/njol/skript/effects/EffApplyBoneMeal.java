...
public class EffApplyBoneMeal extends Effect {
	...
	protected void execute(Event event) {
		int times = 1;
		if (amount != null) {
			Number n = amount.getSingle(event);
			if (n == null)
				return;
			times = n.intValue();
		}
	...
	public String toString(@Nullable Event event, boolean debug) {
		if (amount == null)
			return "apply bone meal to " + blocks.toString(event, debug);
		return "apply " + amount.toString(event, debug) + " bone meal to " + blocks.toString(event, debug);
