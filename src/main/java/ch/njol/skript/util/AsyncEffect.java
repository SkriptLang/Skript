		...
		Bukkit.getScheduler().runTaskAsynchronously(Skript.getInstance(), () -> {
			...
			if (getNext() != null) {
				Bukkit.getScheduler().runTask(Skript.getInstance(), () -> { // Walk to next item synchronously
					Timing timing = null;
					if (Skript.getTimings().isEnabled()) { // getTrigger call is not free, do it only if we must
						Trigger trigger = getTrigger();
						if (trigger != null) {
							timing = Skript.getTimings().start(trigger);
						}
					}
					...
					Skript.getTimings().stop(timing); // Stop timing if it was even started
				});
			} else {
