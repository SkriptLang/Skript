...
public class SkriptParser {
	 ...
	 */
	public static <E extends SyntaxElement, I extends SyntaxInfo<? extends E>> @Nullable E parse(
		String input,
	...
	public static <E extends SyntaxElement> @Nullable E parse(
		String input,
		@NotNull ParsingConstraints parsingConstraints,
		Iterator<? extends SyntaxElementInfo<E>> allowedSyntaxes,
		ParseContext context,
		@Nullable String defaultError
	) {
		input = input.trim();
	 ...
	 */
	@ApiStatus.Internal
	public <E extends SyntaxElement> @Nullable E parse(@NotNull Iterator<? extends SyntaxElementInfo<E>> allowedSyntaxes) {
		allowedSyntaxes = parsingConstraints.constrainIterator(allowedSyntaxes);
		try (ParseLogHandler log = SkriptLogger.startParseLogHandler()) {
			// for each allowed syntax
			while (allowedSyntaxes.hasNext()) {
				SyntaxElementInfo<? extends E> info = allowedSyntaxes.next();
				// check each of its patterns
				for (int patternIndex = 0; patternIndex < info.patterns.length; patternIndex++) {
					log.clear();
					E element = parse(info, patternIndex);
	 ...
	 * @param <E> The type of {@link SyntaxElement} that will be returned.
	 */
	private <E extends SyntaxElement> @Nullable E parse(@NotNull SyntaxElementInfo<? extends E> info, int patternIndex) {
		...
		// construct instance
		try {
			element = info.getElementClass().getDeclaredConstructor().newInstance();
		} catch (InstantiationException | IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
			throw new RuntimeException("Failed to create instance of " + info.getElementClass().getName(), e);
		}
	 ...
	 * @see #parse(SyntaxElementInfo, int)
	 */
	...
	/**
	 * A class that contains information about the parsing of a pattern and what was matched.
	 */
	public static class ParseResult {
		...
		/**
		 * The parsed sub-expressions that this pattern contains.
		 * This is an array of {@link Expression} objects, which may include nulls if the expression 'slot' was nullable and not occupied.
		 */
		public Expression<?>[] exprs;
		/**
		 * Any regexes that were matched in the pattern.
		 */
		...
		public int mark = 0;
		/**
		 * Any tags that were encountered in the pattern.
		 */
		public List<String> tags = new ArrayList<>();

		public ParseResult(@NotNull SkriptParser parser, String pattern) {
			expr = parser.input;
		...
		/**
		 * Whether a specific tag was encountered in the parsed pattern.
		 * @param tag The tag to check for.
		 * @return Whether the tag was encountered.
		 */
