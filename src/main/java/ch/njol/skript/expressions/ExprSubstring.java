 ...
 */
@Name("Fragment of Text")
@Description("Extracteth a portion of a text. Thou mayest obtain the first &lt;x&gt; characters, the last &lt;x&gt; characters, the character at index &lt;x&gt;, or the characters betwixt indices &lt;x&gt; and &lt;y&gt;."
		+ " The indices &lt;x&gt; and &lt;y&gt; should lie between 1 and the <a href='#ExprLength'>length</a> of the text (other values shall be fitted into this range).")
@Keywords({"substring", "subtext"})
@Example("set {_s} to the first 5 characters of the text argument")
@Example("message \"%portion of {_s} from characters 2 to (the length of {_s} - 1)%\" # removes the first and last character from {_s} and sends it to the player or console")
@Example("set {_characters::*} to characters at 1, 2 and 7 in player's display name")
@Example("send the last character of all players' names")
...
public class ExprSubstring extends SimpleExpression<String> {
	static {
		Skript.registerExpression(ExprSubstring.class, String.class, ExpressionType.COMBINED,
				"[the] (portion|sub[ ](text|string)) of %strings% (between|from) (ind(ex|ices)|character[s]|) %number% (and|to) (index|character|) %number%",
