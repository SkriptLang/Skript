...
public class ExprBurnCookTime extends PropertyExpression<Block, Timespan> {
	...
	public void change(Event event, @Nullable Object[] delta, ChangeMode mode) {
        ...
        if (isEvent) {
			...
			burnEvent.setBurnTime((int) value.apply(new Timespan(Timespan.TimePeriod.TICK, burnEvent.getBurnTime())).getTicks());
			return;
		}
		...
		for (Block block : getExpr().getArray(event)) {
			...
			long time = value.apply(new Timespan(Timespan.TimePeriod.TICK, cookTime ? furnace.getCookTime() : furnace.getBurnTime())).getTicks();
