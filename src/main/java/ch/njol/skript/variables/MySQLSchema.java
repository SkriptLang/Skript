...
final class MySQLSchema {
	...
	static void initialize(Connection connection, String table) throws SQLException {
		...
		execute(connection, "CREATE TABLE IF NOT EXISTS `" + table + "` ("
				+ "name_hash BINARY(32) PRIMARY KEY, name LONGTEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL, "
				+ "type VARCHAR(255) NOT NULL, value LONGBLOB) "
	...
	static void migrate(Connection connection, String table, Map<String, String> columns) throws SQLException {
		table = PooledMySQLStorage.identifier(table);
		Map<String, String> expected = new HashMap<>(Map.of("name_hash", "binary(32)", "name", "longtext",
		...
		String alteration = binaryNames
				? "MODIFY COLUMN name LONGTEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL" : "";
		if (compactValues)
			alteration += (binaryNames ? ", " : "") + "DROP COLUMN small_value";
