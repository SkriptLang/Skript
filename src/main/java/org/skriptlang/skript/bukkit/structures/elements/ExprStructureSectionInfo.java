...
import org.skriptlang.skript.bukkit.structures.elements.EffSecStructurePlace.StructurePlaceEvent;
...
public class ExprStructureSectionInfo extends SimpleExpression<Object> {
	...
	protected Object[] get(Event event) {
		if (!(event instanceof StructurePlaceEvent details))
			return new Object[0];
		return switch (setting) {
			case INCLUDES_ENTITIES -> CollectionUtils.array(details.includeEntities());
			case INTEGRITY -> CollectionUtils.array(details.getIntegrity());
	...
	public String toString(@Nullable Event event, boolean debug) {
		return setting.name().toLowerCase(Locale.ENGLISH).replace("_", " ");
	}
