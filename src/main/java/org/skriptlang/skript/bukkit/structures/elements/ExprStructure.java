...
@Description({
	...
	"If the name contains a colon, it'll grab from the Minecraft structure space (Data packs included for namespaces).",
})
@Examples({
...
public class ExprStructure extends SimpleExpression<Structure> {
	...
	public String toString(@Nullable Event event, boolean debug) {
		if (location1 == null || location2 == null)
			return (register ? "registered " : "unregistered ") + "structures " + names.toString(event, debug);
		return "structure from " + location1.toString(event, debug) + " to " + location2.toString(event, debug) + (entities ? " including entities" : "");
	}
