...
public class ExprStructureInfo extends PropertyExpression<Structure, Object> {
	...
	protected Object[] get(Event event, Structure[] source) {
		return switch (property) {
			case BLOCKS -> Arrays.stream(source).flatMap(structure -> {
			...
			case ENTITIES -> get(source, structure -> structure.getEntities().toArray(Entity[]::new));
			case SIZE -> get(source, Structure::getSize);
			case NAME -> {
				Map<NamespacedKey, Structure> structures = Bukkit.getStructureManager().getStructures();
				yield get(source, structure -> structures.entrySet().stream()
