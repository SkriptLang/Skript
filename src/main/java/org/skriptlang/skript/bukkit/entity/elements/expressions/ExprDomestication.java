...
public class ExprDomestication extends SimplePropertyExpression<LivingEntity, Integer> {
	...
	public static void register(SyntaxRegistry registry) {
		registry.register(
			...
			infoBuilder(ExprDomestication.class, Integer.class, "[:max[imum]] domestication level", "livingentities", false)
				.supplier(ExprDomestication::new)
				.build()
		);
	}
		...
		for (LivingEntity entity : getExpr().getArray(event)) {
			if (entity instanceof AbstractHorse horse) {
				...
				switch (mode) {
					...
					case ADD -> level = (int) Math2.addClamped(level, change);
					case REMOVE -> level = (int) Math2.addClamped(level, -change);
					case RESET -> level = 1;
					default -> {
						assert false;
						return;
