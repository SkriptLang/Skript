...
public class EffKnockback extends Effect {
	...
	public static void register(SyntaxRegistry registry) {
		registry.register(
			...
			SyntaxInfo.builder(EffKnockback.class)
				.addPattern("(apply knockback to|knock[back]) %livingentities% [%direction%] [with (strength|force) %-number%]")
				.supplier(EffKnockback::new)
				.build()
		);
	}
	...
	public String toString(@Nullable Event event, boolean debug) {
		...
		builder.append("knockback", entities, direction);
		if (strength != null)
			builder.append("with strength", strength);
