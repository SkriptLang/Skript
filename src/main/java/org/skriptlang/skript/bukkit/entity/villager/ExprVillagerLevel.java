...
public class ExprVillagerLevel extends SimplePropertyExpression<LivingEntity, Number> {
	...
	public static void register(SyntaxRegistry registry) {
		registry.register(
			...
			infoBuilder(ExprVillagerLevel.class, Number.class, "villager (level|:experience)", "livingentities", false)
				.supplier(ExprVillagerLevel::new)
				.build()
		);
	}
	...
	public void change(Event event, Object @Nullable [] delta, ChangeMode mode) {
		...
		for (LivingEntity livingEntity : getExpr().getArray(event)) {
			...
			} else if (newLevel > previousAmount) {
