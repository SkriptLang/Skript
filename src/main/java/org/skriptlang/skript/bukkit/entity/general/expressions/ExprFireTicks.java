...
public class ExprFireTicks extends SimplePropertyExpression<Entity, Timespan> {
	...
	public static void register(SyntaxRegistry registry) {
		registry.register(
			...
			infoBuilder(ExprFireTicks.class, Timespan.class, "[:max[imum]] (burn[ing]|fire) (time|duration)", "entities", false)
				.supplier(ExprFireTicks::new)
				.build()
		);
	}
	...
	public void change(Event event, Object @Nullable [] delta, ChangeMode mode) {
		...
		for (Entity entity : getExpr().getArray(event)) {
			switch (mode) {
				...
				case ADD -> entity.setFireTicks(entity.getFireTicks() + change);
				case REMOVE -> entity.setFireTicks(entity.getFireTicks() - change);
			}
		}
	}
